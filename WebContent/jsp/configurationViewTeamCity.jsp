...
<body>
  ...
  <form:form id="itapConfigDTOForm" name="itapConfigDTOForm" action="${pageContext.request.contextPath}/configurationTeamCity" 
			  ...
			  <c:choose>
	    		  <c:when test="${itapConfigDTOs ne null}">      
 					<table id="tableSort" class="table table-striped table-bordered">
						...
						<tbody>
						   <c:forEach items="${itapConfigDTOs}" var="itapConfigDTOs" varStatus="status">
		                   <tr>
		                   	 <td align="left"><input type="checkbox" name="teamCityJobs" id="check_${status.index}"
										value="${itapConfigDTOs.jobName}_123" onclick="checkToDelete(check_${status.index});" /></td>
		                     <td align="left">${itapConfigDTOs.jobName}</td>
		                     <td align="left">${itapConfigDTOs.apiTest}</td>
		                     <td align="left"><a class="btn btn-success" href="./configurationJobBuildTeamCity?id=${itapConfigDTOs.jobName1}"  
		                     					onclick="loadAjax();">Build</a></td>
		                 	 <td align="left"><a class="btn btn-danger"  id="check_${status.index}_Del"
								 href="./deleteConfiguration?id=${itapConfigDTOs.jobName}&ciEngineName=teamCity&serverId=2"
	...
	$(document).ready(function() {	
	 ...
	 function checkboxCheck(){
  	...
  	function deleteSelectedJobs() {
  		var url = "./deleteMultipleTrain" ;//obj.href;
